...
# Plot 1 - Daily Milk Received Line Chart
fig1 = px.line(month_data, x='Date of Record', y=col_name,
               title='Daily Milk Received',
               markers=True)
st.plotly_chart(fig1, use_container_width=True)

# Plot 2 - Milk Received? Pie Chart
counts = month_data['Milk Received?'].value_counts().reset_index()
counts.columns = ['Milk Received?', 'Count']
fig2 = px.pie(counts, names='Milk Received?', values='Count', title='Milk Received Ratio')
st.plotly_chart(fig2, use_container_width=True)

# Plot 3 - Milk Received Quantity by Status Bar Chart
bar_data = month_data.groupby('Milk Received?')[col_name].sum().reset_index()
fig3 = px.bar(bar_data, x='Milk Received?', y=col_name, title='Milk Received Quantity by Status')
